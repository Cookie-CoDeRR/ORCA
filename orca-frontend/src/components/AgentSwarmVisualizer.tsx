...
export default function AgentSwarmVisualizer() {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto rounded-3xl bg-white border border-zinc-200 p-6 md:p-8 shadow-xl text-zinc-900 relative overflow-hidden">
      {/* Top Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b border-zinc-200 pb-6">
        ...
        <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-100 border border-zinc-200 text-xs font-mono text-zinc-700">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
          <span>Swarm Latency: <strong className="text-blue-600">&lt; 480ms</strong></span>
        </div>
      </div>
        ...
        {AGENTS.map((agent) => {
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between w-full mb-3">
                <div
                  className={`w-9 h-9 rounded-xl flex items-center justify-center border ${
                    isSelected ? "bg-blue-600 text-white border-blue-600" : "bg-zinc-100 text-zinc-600 border-zinc-200"
                  }`}
                >
              ...
              <div>
                <p className="text-xs font-bold text-zinc-900 tracking-tight">{agent.name.split(" ")[0]}</p>
                <p className="text-[10px] text-zinc-500 font-mono mt-0.5">{agent.latency}</p>
              </div>
            </button>
          );
        })}
          ...
          <div className="grid md:grid-cols-12 gap-6 items-start">
            ...
            <div className="md:col-span-7 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-xl font-bold text-zinc-900">{selectedAgent.name}</h4>
                  <p className="text-xs font-mono text-zinc-500">{selectedAgent.role}</p>
                </div>
              </div>
              ...
              {selectedAgent.formula && (
                <div className="p-3.5 rounded-xl bg-white border border-zinc-200 font-mono text-xs text-zinc-900 shadow-xs">
                  <span className="text-[10px] text-zinc-500 uppercase block mb-1 font-semibold">Mathematical Formulation:</span>
                  <span className="font-semibold text-blue-700">{selectedAgent.formula}</span>
                </div>
              )}

              <div className="grid sm:grid-cols-2 gap-3 pt-2">
                <div className="p-3.5 rounded-xl bg-white border border-zinc-200 shadow-xs">
                  <span className="text-[10px] font-mono text-zinc-500 uppercase block mb-1.5 font-semibold">Input Telemetry:</span>
                ...
                <div className="p-3.5 rounded-xl bg-white border border-zinc-200 shadow-xs">
                  <span className="text-[10px] font-mono text-zinc-500 uppercase block mb-1.5 font-semibold">Output Schema:</span>
                  <ul className="text-xs text-zinc-700 space-y-1.5">
            ...
            <div className="md:col-span-5 p-5 rounded-2xl bg-white border border-zinc-200 shadow-xs space-y-4">
              <div className="flex items-center justify-between border-b border-zinc-200 pb-3">
                <div className="flex items-center gap-2 text-xs font-mono text-zinc-700 font-semibold">
              ...
              <div className="space-y-3 font-mono text-xs">
                <div className="flex justify-between py-1 border-b border-zinc-100">
                  <span className="text-zinc-500">Backbone Model:</span>
                  <span className="text-zinc-900 font-semibold text-right">{selectedAgent.model}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-zinc-100">
                  <span className="text-zinc-500">Inference Latency:</span>
                  <span className="text-blue-600 font-bold">{selectedAgent.latency}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-zinc-100">
                  <span className="text-zinc-500">Memory Saver:</span>
                  <span className="text-zinc-700 font-medium">AsyncPostgresSaver</span>
                </div>
                <div className="flex justify-between py-1">
                  <span className="text-zinc-500">Data Isolation:</span>
                  <span className="text-zinc-900 font-bold">100% Air-Gapped</span>
                </div>
              </div>
            </div>
          </div>
